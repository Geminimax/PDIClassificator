...
for image_dir in os.listdir(image_dir_path):
    ...
    if os.path.isdir(full_path):
        ...
        while(current_image_count < image_limit_per_folder):
            ...
            image = io.imread(os.path.join(full_path, list_imgs[img_id]))
            train_set.append(list_imgs.pop(img_id))
            labels.append(image_dir)
            current_image_count += 1
            num_imgs-=1
...
def using_lbp(train_set, test_set, labels):
    radius = 3
    n_points = radius * 8
    
    feature_set = []
    for image in train_set:
        feature_set.append(extract_lbp(image,radius,n_points))
    
    apply_model(feature_set, test_set, labels)

#using_lbp(train_set, test_set, labels)
...
def using_hog(train_set, test_set, labels):

    feature_set = []
    for image in train_set:
        feature_set.append(extract_hog(image))
...
def using_colorHist(train_set, test_set, labels):
    
    feature_set = []
    for image in train_set:
        feature_set.append(extract_colorHist(image))
